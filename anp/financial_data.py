    ...
    def generate(self, base_i, seq_len=10, is_train=True):
        ...
        for t in range(seq_len):
            ...
            for i in range(batch_size):
                ...
                c_y_batch[i] = c_y[c_idx] + torch.randn_like(c_y[c_idx]) * sig
                if is_train:
...
class TimeSeriesStock:
    ...
    def get_timeseries(self, ts_nm='mkt_rf'):
        data = pd.read_csv('./data/kr_close_y.csv', index_col=0)
        base_y = data[ts_nm]
        return base_y
    ...
    def generate(self, base_i, seq_len=10, is_train=True):
        ...
        for t in range(seq_len):
            ...
            for i in range(batch_size):
                # c_idx = np.sort(np.random.choice(np.arange(len_ctx), num_context, replace=False))
                c_idx = np.sort(np.concatenate([np.random.choice(np.arange(len_ctx - 20), num_context - 20, replace=False), np.arange(len_ctx-20, len_ctx)]))
                t_idx = np.concatenate([c_idx, np.arange(len_ctx, len_total)])
