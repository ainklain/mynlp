...
def predict_plot_mtl_cross_section_test2(model, dataset_list, save_dir='out.png', ylog=False, eval_type='pos'):
    if dataset_list is False:
        return False
    else:
        input_enc_list, output_dec_list, target_dec_list, features_list, additional_infos, start_date, end_date = dataset_list
    ...
    for i, (input_enc_t, output_dec_t, target_dec_t) in enumerate(zip(input_enc_list, output_dec_list, target_dec_list)):
        ...
        true_y[t] = np.mean(labels[:, 0, idx_y])
        if eval_type == 'pos_5d':
            value_ = predictions['pos_5d'][:, 0, 0]
        elif eval_type == 'pos_20d':
            value_ = predictions['pos_20d'][:, 0, 0]
        elif eval_type == 'logy':
            value_ = predictions['logy'][:, 0, 0]
        elif eval_type == 'ir20':
            value_ = predictions['logy'][:, 0, 1] / np.abs(predictions['std'][:, 0, 0])
        elif eval_type == 'ir60':
            value_ = predictions['logy'][:, 0, 2] / np.abs(predictions['std'][:, 0, 1])
        elif eval_type == 'ir120':
            value_ = predictions['logy'][:, 0, 3] / np.abs(predictions['std'][:, 0, 2])
