...
def arr_to_dataset_ntasks(log_arr, sampling_freq=20):
    m_days = 500
    k_days = 20
    delay_days = 1
...
def convert_to_image(data_arr, image_size):
    # Gramian Angular Field
    gasf = GramianAngularField(image_size=image_size, method='summation')
...
def preprocessing_ntasks(arr, image_size=64):
    ...
    data_, label_ = arr_to_dataset_ntasks(arr, sampling_freq=5)
    data_arr = np.stack(data_, axis=0)
    label_arr = np.stack(label_, axis=0)
    ...
    figs = convert_to_image(data_arr, image_size).astype(np.float32)
