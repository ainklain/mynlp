...
from anp.financial_data import TimeSeries
...
def main():
    n_epochs = 100
    batch_size = 16
    dataset = TimeSeries(batch_size=256,
                         max_num_context=250,
                         predict_length=60)
    ...
    for epoch_idx in range(n_epochs):
        data_train = dataset.generate(50, seq_len=1, is_train=True)
        data_train.to('cuda:0')

        # Define the loss
        ((c_x, c_y), t_x), t_y = data_train.query, data_train.target_y

        for i in range(len(c_x[0]) // batch_size):
            optim.zero_grad()
            x_shuffled, t_shuffled = c_y[0][batch_size * i: batch_size * (i+1)], c_x[0][batch_size * i: batch_size * (i+1)]
            x_shuffled, t_shuffled = torch.transpose(x_shuffled, 0, 1), torch.transpose(t_shuffled, 0, 1)

            t, sorted_idx = torch.sort(t_shuffled, dim=0)
            x = torch.gather(x_shuffled, dim=0, index=sorted_idx)
